...
#             Optimised for Fly.io (~1 Gbps, Global Edge)
#
#  SETUP:
#  1. Install flyctl: curl -L https://fly.io/install.sh | sh
#  2. fly auth login
#  3. fly launch  (in the folder with this file)
#  4. fly secrets set API_ID=xxx API_HASH=xxx BOT_TOKEN=xxx
#  5. Encode your session:  python encode_session.py
#     fly secrets set SESSION_STRING=<output>
#  6. fly deploy
#
#  MONITOR:
#     fly logs          (live logs)
...
# ── Credentials from Fly.io Secrets ──────────────────────────
# Set with: fly secrets set API_ID=xxx API_HASH=xxx BOT_TOKEN=xxx
...
# Fly.io machines have fast local NVMe SSD at /tmp
...
def load_s():
    if os.path.exists(SETTINGS_FILE):
        ...
        for k, v in DEFAULT_SETTINGS.items():
            data.setdefault(k, v)
        return data
    data = DEFAULT_SETTINGS.copy(); save_s(data); return data
...
def settings_text():
    s=load_s(); ok='Logged in' if (user_client and user_client.is_connected()) else 'Not logged in'
    ...
    return '\n'.join(['**Bot Settings** (Fly.io)','',
        'Login    : '+ok,'Source   : `'+str(s['source_chat'] or 'not set')+'`',
        'Target   : `'+str(s['target_chat'] or 'not set')+'`','',
        'Caption  : `'+cap_label(s)+'`','Rename   : `'+(s['rename_tag'] or 'none')+'`',
        'Replace  :',rw,'Remove   : '+rm,'Thumb    : '+('set' if s['thumbnail'] else 'none'),'',
        'DL conns : `'+str(s['dl_connections'])+'` (parallel streams)','UL chunk : `512 KB` (max, fixed)'])
...
def apply_filters(text,s):
    ...
    for w in s.get('remove_words',[]): (text := text.replace(w,'')) if w else None
    for old,new in s.get('replace_words',{}).items(): (text := text.replace(old,new)) if old else None
    return text.strip()

def make_filename(original,s):
    name,ext=os.path.splitext(original); name=apply_filters(name,s); tag=s.get('rename_tag','').strip()
...
def make_caption(s,msg,fname,size_mb):
    c=s.get('caption','')
    if c==' ': return ''
    if c.strip(): return apply_filters(c.replace('{filename}',fname).replace('{size}',str(round(size_mb,1))+' MB'),s)
...
def doc_size_bytes(msg):
    try:
        if isinstance(msg.media,MessageMediaDocument) and msg.media.document: return msg.media.document.size or 0
    except Exception: pass
    return 0
...
async def fetch_thumb(msg):
    try:
        ...
        best=None; bs=0
        for t in doc.thumbs:
            if isinstance(t,PhotoSize) and t.size>bs: bs=t.size; best=t
            elif isinstance(t,PhotoCachedSize) and len(t.bytes or b'')>bs: bs=len(t.bytes); best=t
        if best is None:
            for t in doc.thumbs:
...
_spd={}
def smooth_spd(key,instant): prev=_spd.get(key,instant); v=0.25*instant+0.75*prev; _spd[key]=v; return v
def fmt_size(b): mb=b/1048576; return (str(round(mb/1024,2))+' GB') if mb>=1024 else (str(round(mb,1))+' MB') if mb>=1 else (str(round(mb*1024))+' KB')
def fmt_eta(s):
...
def bar(pct,w=20): f=int(pct*w); return '['+'#'*f+'-'*(w-f)+']'

def prog_txt(mode,idx,total,cur,tot,t0,key,ok,skip,fname='',streams=1):
    pct=cur/tot if tot>0 else 0; ela=max(time.time()-t0,0.1); spd=smooth_spd(key,(cur/1048576)/ela)
    eta=((tot-cur)/1048576)/spd if spd>0.01 else 0
    arrow='DL' if mode=='dl' else 'UL'; sinfo=(' x'+str(streams)+' streams') if streams>1 else ''
    fname_s=(fname[:28]+'..') if len(fname)>30 else fname
    return '\n'.join(filter(None,[
        '['+arrow+sinfo+'] ['+str(idx)+'/'+str(total)+']  '+str(round(pct*100,1))+'%',
        '`'+bar(pct)+'`', ('`'+fname_s+'`') if fname_s else '',
        fmt_size(cur)+' / '+fmt_size(tot), str(round(spd,2))+' MB/s   ETA '+fmt_eta(eta),
...
def done_txt(idx,total,fname,sz_b,dl_spd,ul_spd,dl_t,ul_t,ok,skip,caption):
    ...
    return '\n'.join(['DONE ['+str(idx)+'/'+str(total)+']','`'+fname_s+'`',
        fmt_size(sz_b)+'   DL '+str(round(dl_spd,1))+' MB/s   UL '+str(round(ul_spd,1))+' MB/s',
        'Total '+fmt_eta(dl_t+ul_t)+cap_line,'done:'+str(ok)+'  skip:'+str(skip)])

def skip_txt(idx,total,mid,reason,ok,skip):
    return 'SKIP ['+str(idx)+'/'+str(total)+'] id:'+str(mid)+'\n`'+reason[:100]+'`\ndone:'+str(ok)+'  skip:'+str(skip)

def final_txt(total,ok,skip,elapsed):
    return '\n'.join(['ALL DONE!','','Total  : '+str(total),'Success: '+str(ok),'Skipped: '+str(skip),'Time   : '+fmt_eta(elapsed)])
...
def make_client(session_path,receive_updates=False):
    return TelegramClient(session_path,API_ID,API_HASH,
        connection=ConnectionTcpFull,connection_retries=3,retry_delay=1,
        auto_reconnect=True,receive_updates=receive_updates,
        flood_sleep_threshold=0,request_retries=1,timeout=30)
...
async def _dl_stream(client,media,file_size,stream_idx,n_streams,out_path,written,lock,t0,progress_cb):
    offset=stream_idx*CHUNK; stride=n_streams*CHUNK; pos=offset
    async for chunk in client.iter_download(media,offset=offset,stride=stride,request_size=CHUNK,file_size=file_size):
        async with lock:
            with open(out_path,'r+b') as fh: fh.seek(pos); fh.write(chunk)
            written[0]+=len(chunk); await progress_cb(written[0],file_size)
        pos+=stride

async def parallel_download(msg,out_path,file_size,n_streams,status_msg,idx,total,ok,skip,fname):
    t0=time.time(); last_t=[0.0]; key='dl'+str(idx); written=[0]; lock=asyncio.Lock()
    async def cb(cur,tot):
        now=time.time()
        if now-last_t[0]<3.5 and cur<tot: return
        last_t[0]=now
        try: await status_msg.edit(prog_txt('dl',idx,total,cur,tot,t0,key,ok,skip,fname,n_streams),buttons=cancel_kb(),parse_mode='markdown')
        except Exception: pass
    with open(out_path,'wb') as fh: fh.seek(file_size-1); fh.write(b'\x00')
    media=msg.media.document if isinstance(msg.media,MessageMediaDocument) else msg.media.photo
    clients=[]
    for i in range(n_streams): c=make_client(USER_SESSION); await c.connect(); clients.append(c)
    try: await asyncio.gather(*[_dl_stream(clients[i],media,file_size,i,n_streams,out_path,written,lock,t0,cb) for i in range(n_streams)])
    finally:
        for c in clients:
...
async def simple_download(msg,out_path,file_size,status_msg,idx,total,ok,skip,fname):
    ...
    async def cb(cur,tot):
        ...
        if now-last_t[0]<3.5 and cur<tot: return
        last_t[0]=now
        try: await status_msg.edit(prog_txt('dl',idx,total,cur,tot,t0,key,ok,skip,fname),buttons=cancel_kb(),parse_mode='markdown')
        except Exception: pass
...
async def download_file(msg,status_msg,idx,total,ok,skip,fname,file_size):
    ...
    use_parallel=file_size>=PARALLEL_DL_MIN and isinstance(msg.media,MessageMediaDocument) and n_streams>1
    out_path=os.path.join(DOWNLOAD_DIR,'dl_'+str(idx)+'_'+str(int(time.time()))+(os.path.splitext(fname)[1] or '.bin'))
    last_err=''
    for attempt in range(1,4):
        try:
            result=await(parallel_download(msg,out_path,file_size,n_streams,status_msg,idx,total,ok,skip,fname)
                         if use_parallel else simple_download(msg,out_path,file_size,status_msg,idx,total,ok,skip,fname))
            ...
            return path,None
        except FloodWaitError as fe: last_err='FloodWait '+str(fe.seconds)+'s'; await asyncio.sleep(fe.seconds+2); await check_connected()
        except asyncio.CancelledError: raise
        except Exception as ex:
            last_err=str(ex)
            if attempt<3:
                use_parallel=False
                try: await status_msg.edit('DL retry '+str(attempt)+'/3\n`'+last_err[:100]+'`',buttons=cancel_kb(),parse_mode='markdown')
                except Exception: pass
                await asyncio.sleep(attempt*3); await check_connected()
    return None,last_err

async def upload_file(path,target,caption,thumb,is_photo,vw,vh,vd,status_msg,idx,total,ok,skip):
    import mimetypes as _mt
    sz_b=os.path.getsize(path); sz_mb=sz_b/1048576; t0=time.time(); last_t=[0.0]
    key='ul'+str(idx); fname=os.path.basename(path)
    ...
    async def cb(cur,tot):
        ...
        if now-last_t[0]<3.5 and cur<tot: return
        last_t[0]=now
        try: await status_msg.edit(prog_txt('ul',idx,total,cur,tot,t0,key,ok,skip,fname),buttons=cancel_kb(),parse_mode='markdown')
        except Exception: pass
    file_handle=await user_client.upload_file(path,part_size_kb=512,file_name=fname,progress_callback=cb)
    attrs=[DocumentAttributeFilename(file_name=fname)]
    if not send_as_photo and vd>0: attrs.append(DocumentAttributeVideo(duration=vd,w=vw,h=vh,supports_streaming=True))
    thumb_handle=None
    if thumb and not send_as_photo:
        try: thumb_handle=await user_client.upload_file(thumb,part_size_kb=64)
        except Exception: pass
    await user_client.send_file(target,file=file_handle,caption=caption,thumb=thumb_handle,
                                 supports_streaming=(not send_as_photo and vd>0),
                                 force_document=(not send_as_photo),mime_type=_mime,attributes=attrs)
    return sz_mb/max(time.time()-t0,0.1)


# =============================================================
#  TRANSFER + DELETE ENGINE (shared logic)
# =============================================================

async def run_transfer(uid,source,msg_ids,status_msg):
    s=load_s(); total=len(msg_ids); ok_count=0; skip_count=0; run_start=time.time()
    try: target=int(s['target_chat'])
    except: await status_msg.edit('Target not set.',buttons=main_kb(),parse_mode='markdown'); return
    if not await check_connected(): await status_msg.edit('Not connected.',buttons=main_kb(),parse_mode='markdown'); return
    for idx,mid in enumerate(msg_ids,1):
        if cancel_flags.get(uid):
            cancel_flags.pop(uid,None)
            await status_msg.edit(final_txt(total,ok_count,skip_count,time.time()-run_start)+'\n(Cancelled)',buttons=main_kb(),parse_mode='markdown'); return
        try: await status_msg.edit('Fetching ['+str(idx)+'/'+str(total)+'] id:'+str(mid)+'\ndone:'+str(ok_count)+'  skip:'+str(skip_count),buttons=cancel_kb(),parse_mode='markdown')
        except Exception: pass
        msg=None; fetch_err=''
        for attempt in range(1,4):
            try: msg=await asyncio.wait_for(user_client.get_messages(source,ids=mid),timeout=20.0); break
            except asyncio.TimeoutError: fetch_err='timeout'; await check_connected()
            except FloodWaitError as fe: fetch_err='FloodWait '+str(fe.seconds)+'s'; await asyncio.sleep(fe.seconds+2); await check_connected()
            except Exception as ex: fetch_err=str(ex)[:80]; await asyncio.sleep(3); await check_connected()
        if msg is None:
            skip_count+=1
            try: await status_msg.edit(skip_txt(idx,total,mid,'Fetch failed: '+fetch_err,ok_count,skip_count),buttons=cancel_kb(),parse_mode='markdown')
            except Exception: pass
            continue
        is_photo=isinstance(msg.media,MessageMediaPhoto); is_doc=isinstance(msg.media,MessageMediaDocument)
        if not is_photo and not is_doc:
            if msg.message and msg.message.strip():
                cap=apply_filters(msg.message,s); custom=s.get('caption','')
                if custom and custom!=' ': cap=apply_filters(custom.replace('{filename}','').replace('{size}',''),s)
        ...
        if not fpath:
            skip_count+=1
            try: await status_msg.edit(skip_txt(idx,total,mid,'DL failed: '+str(err),ok_count,skip_count),buttons=cancel_kb(),parse_mode='markdown')
            except Exception: pass
            continue
        dl_time=max(time.time()-dl_start,0.1); sz_b=os.path.getsize(fpath); sz_mb=sz_b/1048576; dl_spd=sz_mb/dl_time
        if cancel_flags.get(uid):
            safe_del(fpath); cancel_flags.pop(uid,None)
            await status_msg.edit(final_txt(total,ok_count,skip_count,time.time()-run_start)+'\n(Cancelled)',buttons=main_kb(),parse_mode='markdown'); return
        raw_clean=re.sub(r'[\\/:<>|*?"]+','_',(orig_name or os.path.basename(fpath))).strip()
        new_name=make_filename(raw_clean,s); new_path=os.path.join(DOWNLOAD_DIR,new_name)
        ...
        for attempt in range(1,4):
            try:
                await upload_file(fpath,target,caption,thumb,is_photo,vw,vh,vd,status_msg,idx,total,ok_count,skip_count)
                upload_ok=True; break
            except FloodWaitError as fe: upload_err='FloodWait '+str(fe.seconds)+'s'; await asyncio.sleep(fe.seconds+2); await check_connected()
            except asyncio.CancelledError: safe_del(fpath); (safe_del(thumb) if thumb_temp else None); raise
            except Exception as ex:
                upload_err=str(ex)
                if attempt<3:
                    try: await status_msg.edit('UL retry '+str(attempt)+'/3\n`'+upload_err[:100]+'`',buttons=cancel_kb(),parse_mode='markdown')
                    except Exception: pass
                    await asyncio.sleep(attempt*3); await check_connected()
        safe_del(fpath)
        ...
        if upload_ok:
            ...
            try: await status_msg.edit(done_txt(idx,total,new_name,sz_b,dl_spd,sz_mb/ul_time,dl_time,ul_time,ok_count,skip_count,caption),buttons=cancel_kb(),parse_mode='markdown')
            except Exception: pass
        else:
            skip_count+=1
            try: await status_msg.edit(skip_txt(idx,total,mid,'UL failed: '+upload_err,ok_count,skip_count),buttons=cancel_kb(),parse_mode='markdown')
            except Exception: pass
    await status_msg.edit(final_txt(total,ok_count,skip_count,time.time()-run_start),buttons=main_kb(),parse_mode='markdown')
...
async def run_delete(uid,source,msg_ids,status_msg):
    ...
    for idx,mid in enumerate(msg_ids,1):
        if cancel_flags.get(uid):
            cancel_flags.pop(uid,None)
            await status_msg.edit('Cancelled.\nDeleted:'+str(deleted)+'  Failed:'+str(failed),buttons=main_kb(),parse_mode='markdown'); return
        try: await status_msg.edit('Deleting ['+str(idx)+'/'+str(total)+'] id:'+str(mid)+'\nDeleted:'+str(deleted)+'  Failed:'+str(failed),buttons=cancel_kb(),parse_mode='markdown')
        except Exception: pass
    ...
    await status_msg.edit('Delete done!\nTotal:'+str(total)+'  Deleted:'+str(deleted)+'  Failed:'+str(failed),buttons=main_kb(),parse_mode='markdown')


# =============================================================
#  KEYBOARDS + BOT + HANDLERS (identical to other platforms)
# =============================================================
...
async def on_cb(e):
    ...
    if data=='cancel':
        ...
        return
    if data=='login':
        txt=('Logged in: '+((await user_client.get_me()).first_name)+'\nReady.') if user_client and user_client.is_connected() else 'Not connected.'
        await e.edit(txt,buttons=back_kb(),parse_mode='markdown'); return
    if data=='src': user_states[uid]='set_src'; await e.edit('Set Source\n\nSend a t.me link or numeric chat ID:',buttons=back_kb()); return
    if data=='tgt': user_states[uid]='set_tgt'; await e.edit('Set Target\n\nSend numeric chat ID (e.g. -1001234567890):',buttons=back_kb()); return
    if data=='cap': user_states[uid]='set_cap'; await e.edit('Caption\n\nCurrent: `'+cap_label(s)+'`\n\nNONE=original | EMPTY=no caption',buttons=back_kb(),parse_mode='markdown'); return
    if data=='ren': user_states[uid]='set_ren'; await e.edit('Rename Tag\n\nCurrent: `'+(s['rename_tag'] or 'none')+'`\n\nSend tag or NONE:',buttons=back_kb(),parse_mode='markdown'); return
    if data=='rep':
    ...
    if data=='sendmsg':
        ...
        await e.edit('Send Message\n\nTarget: `'+str(s['target_chat'])+'`\n\nSend your message:',buttons=back_kb(),parse_mode='markdown'); return
...
async def on_msg(e):
    ...
    if state=='set_src':
        ...
        s['rename_tag']='' if text.upper()=='NONE' else text; save_s(s); user_states.pop(uid,None)
        ...
        if e.photo:
            ...
            s['thumbnail']=p;save_s(s);user_states.pop(uid,None);await e.respond('Thumbnail saved.',buttons=main_kb())
        ...
        if step=='link1':
            ...
            except Exception as ex: await e.respond('Failed: '+str(ex)[:150],buttons=main_kb())
...
async def main():
    global user_client
    print('='*55)
    print('  TELEGRAM RE-UPLOAD BOT  —  Fly.io')
    print('='*55)

    if not API_ID or not API_HASH or not BOT_TOKEN:
        print('[ERROR] Set secrets with:')
        print('  fly secrets set API_ID=xxx API_HASH=xxx BOT_TOKEN=xxx SESSION_STRING=xxx')
        return
    ...
    if session_b64:
        ...
        with open(sf,'wb') as fh: fh.write(base64.b64decode(session_b64))
        print('[OK] Session loaded from SESSION_STRING secret')
    elif not os.path.exists(USER_SESSION+'.session'):
        print('[ERROR] No session found.')
        print('→ Run login.py locally, then: python encode_session.py')
        print('→ fly secrets set SESSION_STRING=<output>')
        return

    try:
        uc=make_client(USER_SESSION); await uc.connect()
        if not await uc.is_user_authorized():
            await uc.disconnect(); print('[ERROR] Session expired.'); return
        user_client=uc; me=await uc.get_me()
        print(f'[OK] User  : {me.first_name} (@{me.username or "N/A"})')
        print(f'[OK] DL    : {DL_CONNECTIONS} parallel streams × 512 KB chunks')
    except Exception as ex: print(f'[ERROR] {ex}'); return

    try:
        await bot.start(bot_token=BOT_TOKEN); me=await bot.get_me()
        print(f'[OK] Bot   : @{me.username}')
        print(f'\n✅ Bot running on Fly.io! Open @{me.username} → /start\n')
    except Exception as ex: print(f'[ERROR] Bot: {ex}'); return
